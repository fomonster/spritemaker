...
uses
  Classes, SysUtils,  RichView, RVStyle, SSXMLUnit,SimSpin, SSColor, uSimpleObjects, Dialogs;
...
begin
  ...
  if ParamType=1 then begin
    Node:=TGeneratorNode(owner.GeneratorNodePtr);
    IO := TIntObj(cCB.Items.Objects[cCB.ItemIndex]);
    int_value^ := IO.I;
    ShowMessage('>> ' + IntToStr(int_value^));
    Node.IsNeedChange:=true;
    IsDocSaved:=false;
  end;
end;
    ...
    if param.ParamType =0 then begin
      ...
      while s<>'' do begin
        ...
        if k>0 then begin
          ...
          v:=copy(v,1,k-1);
          param.cCB.AddItem(v, TIntObj.Create(StrToIntDef(d,j)));
        end else begin
          param.cCB.AddItem(v, TIntObj.Create(j));
        end;
        inc(j);
      end;
