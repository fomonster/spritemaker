...
begin
  ...
  if trim(Serial)<>'' then begin
    ...
    xml:=TXMLNode.Create;
    xml.LoadFromXMLString(s);
    if (xml['RegKey'].Attribute_Str('AppId')='SM2009') and (xml['RegKey'].Attribute_Str('AppName')='SpriteMaker') then begin
...
begin
  ...
  n1:=xml.SubNodes_Add;
  n1.name:='RegKey';
  n1.Attribute_Add('AppId')^.Value:='SM2009';
  n1.Attribute_Add('AppName')^.Value:='SpriteMaker';
...
begin
 ...
 // exit;
  ...
  if (Paramcount>0) and (CryptoKey3=paramstr(1)) then begin
    ...
    xml:=TXMLNode.Create;
    xml.LoadFromXMLString(s);
    SecretDate:=trim(xml['TextureGenerator'].Attribute_Str('InstallDate'));
//    SecretDiskSerial:=xml['TextureGenerator'].Attribute_Str('HardDiskSerial');
    xml.Destroy;
    rfl.Destroy;
    ...
    LoadRegistration;
    //
    if ((StrToDate(SecretDate)<Date) and (Date-StrToDate(SecretDate)>trialDays)) and not CheckRegistration then begin
      Serial:='';
      While trim(Serial)='' do begin
    ...
    if not CheckRegistration then begin
      SerialName:='. Not registered. You have trial period '+IntToStr(trialDays-round(Date-StrToDate(SecretDate)))+' days. Visit http://spritepacker.com to get serial.';
