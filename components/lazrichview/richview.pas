{*******************************************************************************
  ...
  Подправленный мной компонент для работы с RichView
...
type
  ...
  public
    ...
    procedure AddBreak;
    procedure AddBreakLine(_color:TColor;_hsize:Integer);
    ...
    procedure AddBulletCenter(imgNo: Integer; lst: TImageList; fromnewline: Boolean);
    procedure AddControl(ctrl: TControl; center: Boolean);
    procedure MergeControl(ctrl: TControl; center: Boolean);
...
{-------------------------------------}
procedure TRichView.AddBreakLine(_color:TColor;_hsize:Integer);
var info: TLineInfo;
begin
  info := TLineInfo.Create;
  info.StyleNo := -1;
...
procedure TRichView.AddBulletCenter(imgNo: Integer; lst: TImageList; fromnewline: Boolean);
var info: TLineInfo;
begin
  info := TLineInfo.Create;
  info.StyleNo := -6;
...
{-------------------------------------}
procedure TRichView.MergeControl(ctrl: TControl; center: Boolean); { do not free ctrl! }
var info: TLineInfo;
begin
  info := TLineInfo.Create;
  info.StyleNo := -5;
...
{*******************************************************************************
  Форматирование расположения всех или группы строчек
  
*******************************************************************************}
procedure TRichView.Format_(OnlyResized:Boolean; depth: Integer; Canvas: TCanvas;OnlyTail: Boolean);
var i: Integer;
    x,b,d,a: Integer;
    mx : Integer;
...
begin
   ...
   if (csDesigning in ComponentState) or not Assigned(FStyle) or skipformatting or (depth>1) then exit;
...
{*******************************************************************************
  ...
  Тут запутанные параметры в процедуре. Вот что они означают
  
  x - абсцисса левого верхн. угла элемента. При входе - текущего элемента,
      при выходе следующего элемента.

  baseline - ордината левого верхн. угла элемента. При входе - предыдущего.
      При выходе текущего. Обратите внимание (предыдущего и текущего)!!!!!!

  prevdesc - ширина пустой полоски сверху от предыдущего элемента.
  
  prevabove - ширина предыдущего элемента.
  
*******************************************************************************}
procedure TRichView.FormatLine(no: Integer; var x,baseline,prevdesc,prevabove:Integer; Canvas: TCanvas;var sad:TScreenAndDevice);
...
{$ENDIF}
    ...
    wsize,dxsize,lno: Integer; // Ширина всех элементов по X
begin
  width := TextWidth;
  case TLineInfo(lines.Objects[no]).StyleNo of
   -5: { Control }
     begin
       ...
       if  not TLineInfo(lines.Objects[no]).SameAsPrev then begin {новая линия}

         x:=0;
         y := baseline + prevdesc + 1;
         ...
         end else info.Left := sad.LeftMargin;
...
//         info.Left := x+sad.LeftMargin;
         ...
         inc(x,info.Width+1);
       end else begin {продолжаем линию}
         if prevabove < ctrlh+1 then begin
              j := drawlines.Count-1;
              if j>=0 then
                  repeat
                    inc(TDrawLineInfo(drawlines.Objects[j]).Top,ctrlh-prevabove-1);
                    dec(j);
         ...
         if TLineInfo(lines.Objects[no]).Center then begin
           ...
           if j>=0 then begin
             repeat
               inc(wsize,TDrawLineInfo(drawlines.Objects[j]).Width);
               dec(j);
             until TDrawLineInfo(drawlines.Objects[j+1]).FromNewLine;
           end;

           if wsize>width then wsize:=width;
           wsize:=(width-wsize-sad.LeftMargin) div 2;
           j := drawlines.Count-1;
           if j>=0 then begin
             repeat
//               TDrawLineInfo(drawlines.Objects[j]).Left:=
               dxsize:=wsize-TDrawLineInfo(drawlines.Objects[j]).centrdx;
               TDrawLineInfo(drawlines.Objects[j]).centrdx:=dxsize;
               inc(TDrawLineInfo(drawlines.Objects[j]).Left,dxsize);
               lno:=TDrawLineInfo(drawlines.Objects[j]).LineNo;
               if TLineInfo(lines.Objects[lno]).StyleNo = -5 then begin
                 TControl(TLineInfo(lines.Objects[lno]).gr).Left:=TDrawLineInfo(drawlines.Objects[j]).Left;
               end;
...
//       info.Left := x+1+sad.LeftMargin;
       ...
       if TLineInfo(lines.Objects[no]).Center then begin
         ...
         if info.Left<0 then info.Left := 0;
         inc(info.Left,sad.LeftMargin);
       end else info.Left := x+1+sad.LeftMargin;
   ...
   -1: { break line}
    begin
       y5          := MulDiv(TLineInfo(lines.Objects[no]).HSize, sad.ppiyDevice, sad.ppiyScreen);
       info        := TDrawLineInfo.Create;
...
{*******************************************************************************
  Процедурка прорисовки страницы
...
var i,no, yshift, xshift: Integer;
    ...
    {$ENDIF}
begin
 // Если компонент прорисовывается в режиме редактирования (в среде)
 if (csDesigning in ComponentState) or not Assigned(FStyle) then begin
    ...
    if (csDesigning in ComponentState) then
      Canvas.TextOut(ClientRect.Left+1, ClientRect.Top+1, ' Sim3D RichView')
    else
 ...
 // Иначе компонент прорисовывается в программе
 ...
 // Создаем временную картинку buffer
 ...
 for i:= GetFirstVisible(r.Top) to drawlines.Count-1 do begin
   ...
   if no = -1 then begin {break line}
      canv.Pen.Color := li.Color;//FStyle.TextStyles[0].Color;
//      canv.Rectangle(dli.Left+5-xshift, dli.Top+5-yshift,XSize-5-xshift-FRightMargin, dli.Top+10-yshift);
      canv.MoveTo(dli.Left+0-xshift, dli.Top+5-yshift);
      canv.LineTo(XSize-0-xshift-FRightMargin, dli.Top+5-yshift);
{      canv.MoveTo(dli.Left+5-xshift, dli.Top+6-yshift);
      canv.LineTo(XSize-5-xshift-FRightMargin, dli.Top+6-yshift);}
   end;
   { controls ignored }
 end;
...
{*******************************************************************************
  Прорисовка фона.
*******************************************************************************}
procedure TRichView.DrawBackground(DC: HDC; Rect: TRect; Width,Height:Integer);
var i, j: Integer;
    hbr: HBRUSH;
begin
